import { eq, asc, max, and, inArray, isNull } from 'drizzle-orm'
...
export interface NestedQuestionRow extends QuestionRow {
  children: QuestionRow[]
}

export async function createQuestion(
  tenantId: string,
  data: { surveyId: string; text: string; answerType: QuestionRow['answerType']; pointValue: number; scoredMinValue?: number | null; scoredMaxValue?: number | null; isCritical: boolean; parentQuestionId?: string | null },
): Promise<{ question: QuestionRow } | { error: 'survey_not_found' | 'parent_is_child' }> {
  ...
  if (data.parentQuestionId) {
    const [parent] = await db
      ...
      .from(questions)
      .where(eq(questions.id, data.parentQuestionId))
    if (parent?.parentQuestionId !== null && parent?.parentQuestionId !== undefined) {
      return { error: 'parent_is_child' }
    }
  }

  return withTenant(tenantId, async (tx) => {
    const parentFilter = data.parentQuestionId
      ? eq(questions.parentQuestionId, data.parentQuestionId)
      : isNull(questions.parentQuestionId)
    const result = await tx
      .select({ maxPos: max(questions.position) })
      .from(questions)
      .where(and(eq(questions.surveyId, data.surveyId), parentFilter))
    ...
    const [question] = await tx
      ...
      .values({
        ...
        scoredMinValue: data.scoredMinValue ?? null,
        scoredMaxValue: data.scoredMaxValue ?? null,
...
export async function listQuestions(tenantId: string, surveyId: string): Promise<NestedQuestionRow[]> {
  ...
  for (const row of rows) {
    if (row.parentQuestionId) {
      const siblings = byParent.get(row.parentQuestionId) ?? []
      siblings.push(row)
      byParent.set(row.parentQuestionId, siblings)
    }
  }
...
export async function reorderQuestions(
  ...
  return withTenant(tenantId, async (tx) => {
    const existing = await tx
      .select({ id: questions.id, parentQuestionId: questions.parentQuestionId })
      .from(questions)
    ...
    const firstParent = existing[0]!.parentQuestionId
    if (existing.some(r => r.parentQuestionId !== firstParent)) return { error: 'invalid_ids' }
